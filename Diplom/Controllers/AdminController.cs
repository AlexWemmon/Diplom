...
namespace Diplom.Controllers
{
    public class AdminController : Controller
    {
        ...
        public IActionResult user()
        {
            ...
            var tutor = new Tutor();
            foreach (var item in _context.Tutors) { if (item.LogIn == UserName) { tutor = item; } }
            return View(tutor);
        }
        ...
        public async Task<IActionResult> AddFromFile(IFormFile file)
        {
            ...
            Test1 newTes;
            using (var fileStream = new FileStream(Path.Combine(dir, "file.json"), FileMode.Create, FileAccess.ReadWrite))
            {
        ...
        public IActionResult DetailsStudent(int? id)
        {
            ...
            var student = _dataManager._studRepository.GetStudentsById(id.Value);
            return View(student);
        }
        ...
        {
            if (id == null) return NotFound();
            var student = _dataManager._studRepository
        ...
        {
            if (id == null) return NotFound();
            var stud = _dataManager._studRepository
                .GetStudentsById(id.Value);
            return View(stud);
        }
...

        {
            if (users != null)
            {
                ...
                foreach (var item in quests)
                {
                    ...
                    {
                        if (item2.QuestId == item) { questions.Add(item2); }
                    }
                }
                foreach (var item in questions)
                {
                    ...
                    {
                        if (item2.TestId == item.TestId) { Test1.Add(item2); }
                    }
                }
                var Test12 = Test1;
